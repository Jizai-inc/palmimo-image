...
`v0.1.5`), `--pigen-ref` (pin override for one-off builds), `--dry-run`
...
```bash
PI_HOST=user@<addr> PORTAL_TAG=v0.1.5 apply-pi.sh
PI_HOST=user@<addr> PORTAL_TAG=v0.1.5 apply-pi.sh --identity ./palmimo-identity.json
PI_HOST=user@<addr> PORTAL_TAG=v0.1.5 apply-pi.sh --no-apt   # faster re-run once apt is done
```
